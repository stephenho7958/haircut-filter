...
while(True):
    ...
    if face_detection.detections:
        ...
        old_mask = createMask(ymin=int(face_detection.detections[0].location_data.relative_bounding_box.ymin*1920),
...
old_mask_gray = cv.cvtColor(old_mask, cv.COLOR_BGR2GRAY)

p0 = cv.goodFeaturesToTrack(old_gray, mask = old_mask_gray, useHarrisDetector= True, **feature_params)
...
while(1):
    ...
    if(count%20==0):
        p0 = cv.goodFeaturesToTrack(old_gray, mask = face_mask_gray, useHarrisDetector= True, **feature_params)
